...
std::vector<int> loopTail(Link<T> *head) {
    ...
    // Link<T> slow = *head; // if copy constructor is properly defined
    std::vector<int> res;
    if (head == nullptr) {
        res.push_back(0);
        res.push_back(0);
    ...
    // cout << "test 2" << loop << endl;
    cout << "test 2" << endl;
    // (2) get length of loop
    if (loop == 0) {
        res.push_back(0);
    } else {
        // if loop exists (slow == fast) 
        fast = fast->next;
        loop_length++;
        while (fast != slow) {
            fast = fast->next;
...
std::vector<int> josephus(int n, int k) {
    ...
    for (int i=n-1; i>=0; i--) {
        //Link<int>* a = new Link<int>(0, nullptr);
        if (i == n - 1) {   
    ...
    while(length > 0) {
        std::cout << " counter: " << counter << " length: " << length << std::endl;
        curr = nex;
        nex = nex->next;
        if (del == 0) {
            prev = prev->next;
